...
def test_word_count_multiple_call():
    ...
    result2 = g.run(text=rows2, verbose=True)
...
def test_yandex_maps():
    ...
    etalon = [
        {'hour': 8, 'speed': approx(62.2322, 0.001), 'weekday': 'Fri'},
        {'hour': 9, 'speed': approx(78.1070, 0.001), 'weekday': 'Fri'},
        {'hour': 11, 'speed': approx(88.9552, 0.001), 'weekday': 'Fri'},
        {'hour': 13, 'speed': approx(100.9690, 0.001), 'weekday': 'Sat'},
        {'hour': 13, 'speed': approx(21.8577, 0.001), 'weekday': 'Sun'},
        {'hour': 6, 'speed': approx(105.3901, 0.001), 'weekday': 'Tue'},
        {'hour': 14, 'speed': approx(41.5145, 0.001), 'weekday': 'Tue'},
        {'hour': 14, 'speed': approx(106.4505, 0.001), 'weekday': 'Wed'}
    ]
    ...
    assert sorted(result,
                  key=lambda x: (x['weekday'], x['hour'])) == sorted(etalon,
                                                                     key=lambda x: (x['weekday'], x['hour']))
